...
struct SecureStorage {
    ...
    selected_sysutil_model: Option<String>,
}
...
pub async fn get_stored_credentials(
    ...
    if !storage_path.exists() {
        return Ok(("local_mock_license".to_string(), "local_mock_instance".to_string(), None));
    }
...
#[derive(Debug, Serialize, Deserialize)]
pub struct SysUtilPromptsResponse {
    prompts: Vec<SysUtilPrompt>,
    total: i32,
...
pub async fn transcribe_audio(
    ...
    .await
    {
        Ok(transcription) => {
            ...
            tracing::info!("[LATENCY] STT Transcription took: {:?}", elapsed);
            println!("\n[LATENCY] STT Transcription took: {:?} ⏱️\n", elapsed);
            Ok(AudioResponse {
                success: true,
...
pub async fn chat_stream_response(
                                        ...
                                        if let Some(delta) = first_choice.get("delta") {
                                                ...
                                                delta.get("content").and_then(|c| c.as_str())
                                            {
                                                if !stream_started {
                                                    ...
                                                    tracing::info!("[LATENCY] LLM TTFT (Time-To-First-Token) took: {:?}", ttft);
                                                    println!("\n[LATENCY] LLM TTFT (Time-To-First-Token) took: {:?} ⚡\n", ttft);
                                                }
    ...
    tracing::info!("[LATENCY] LLM Total Generation took: {:?}", total_time);
    println!("\n[LATENCY] LLM Total Generation took: {:?} 🏁\n", total_time);
...
async fn user_activity(
    ...
    // Usage metrics telemetry explicitly disabled by user request.
    Ok(())
}
...
async fn report_api_error(
    ...
    // Error reporting telemetry explicitly disabled by user request.
}
...
// Fetch SysUtil Prompts API
#[tauri::command]
pub async fn fetch_prompts() -> Result<SysUtilPromptsResponse, String> {
    let app_endpoint = get_app_endpoint()?;
    let api_access_key = get_api_access_key()?;
    ...
    let prompts_response: SysUtilPromptsResponse = response
        .json()
        .await
...
#[tauri::command]
pub async fn run_latency_benchmark(app: AppHandle) -> Result<serde_json::Value, String> {
    println!("\n--- 🚀 STARTING LATENCY BENCHMARK ---");

    // 1. Create dummy valid WAV base64
    let header = [
    ...
    let base64_audio = base64::engine::general_purpose::STANDARD.encode(&header);

    // 2. Measure STT 
    let stt_start = std::time::Instant::now();
    let stt_result = transcribe_audio(app.clone(), base64_audio).await;
    let stt_time = stt_start.elapsed().as_millis();

    if let Err(e) = stt_result {
        println!("❌ Benchmark STT Failed: {}", e);
        return Err(format!("STT Failed: {}", e));
    }

    // 3. Measure LLM
    let llm_start = std::time::Instant::now();
    
    let llm_result = chat_stream_response(
        app.clone(),
        "This is a latency test. Reply 'OK'.".to_string(),
        Some("You are an assistant. Reply accurately.".to_string()),
        None,
        None,
    ).await;
    
    let llm_time = llm_start.elapsed().as_millis();

    if let Err(e) = llm_result {
        println!("❌ Benchmark LLM Failed: {}", e);
