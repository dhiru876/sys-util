// SysUtil AI Speech Detection, and capture system audio (speaker output) as a stream of f32 samples.
...
impl Default for VadConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            hop_size: 1024,
            sensitivity_rms: 0.022, // Higher threshold — ignores quiet background voices
            peak_threshold: 0.055,  // Requires a clear voice peak — filters ambient sound
            silence_chunks: 90,     // ~2.0s of silence before stopping (more natural)
            min_speech_chunks: 15,  // ~0.3s minimum — filters short background blips
            pre_speech_chunks: 12,  // ~0.27s — enough to catch word start
            noise_gate_threshold: 0.008, // Stronger gate — cuts more background
