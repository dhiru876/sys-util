...
/// Maximum width (in pixels) for screenshots sent to AI.
/// Resizing from 2560 → 1280 reduces base64 payload by ~75%
/// while keeping code/text fully readable for the model.
const SCREENSHOT_MAX_WIDTH: u32 = 1280;

/// Captures the screen and returns a resized base64-encoded PNG.
/// This is the command called by the Voice Mode screenshot button.
/// Unlike `capture_to_base64`, this resizes the image first to prevent
/// API timeouts caused by oversized payloads.
#[tauri::command]
pub async fn capture_screenshot(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        // Find the best monitor (most overlap with current window)
        let mut best_idx: Option<usize> = None;
        let mut best_area: i64 = 0;
        ...
        // Fall back to nearest monitor by center distance
        let target_idx = if let Some(idx) = best_idx {
            idx
        } else {
            let mut closest_idx = 0usize;
        ...
        // Resize to max SCREENSHOT_MAX_WIDTH wide to keep the payload small.
        // This cuts payload size by ~75% on 2560px Retina displays while keeping
        // code/text fully readable for the AI model.
        let resized = if image.width() > SCREENSHOT_MAX_WIDTH {
            let scale = SCREENSHOT_MAX_WIDTH as f32 / image.width() as f32;
            let new_height = (image.height() as f32 * scale) as u32;
            image::imageops::resize(&image, SCREENSHOT_MAX_WIDTH, new_height, FilterType::Lanczos3)
        } else {
            image.into()
        };
