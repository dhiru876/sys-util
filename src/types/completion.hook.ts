...
export interface UseCompletionReturn {
  ...
  resizeWindow: (expanded: boolean, conversationMode?: boolean) => Promise<void>;
  /** Whether screenshot is waiting for voice input */
  isAwaitingVoiceForScreenshot: boolean;
  ...
  /** Called by ScreenshotVoiceCapture when voice is done (or timed out) */
  onScreenshotVoiceCaptured: (voiceText?: string) => Promise<void>;
  /** Array of all completed AI responses in this session (for Shift+A/S navigation) */
  responseHistory: string[];
  /** Current index in responseHistory (-1 = live/latest response) */
  historyIndex: number;
  /** Function to set the history index */
