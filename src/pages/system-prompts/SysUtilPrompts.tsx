...
interface SysUtilPromptsResponse {
  prompts: SysUtilPrompt[];
  total: number;
  last_updated?: string;
}
...
const SELECTED_SYSUTIL_MODEL_STORAGE_KEY = "selected_sysutil_model";
const SELECTED_SYSUTIL_PROMPT_STORAGE_KEY = "selected_sysutil_prompt";

export const SysUtilPrompts = () => {
  const {
    setSystemPrompt,
    hasActiveLicense,
    setSupportsImages,
  ...
  const [prompts, setPrompts] = useState<SysUtilPrompt[]>([]);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [lastUpdated, setLastUpdated] = useState<string | null>(null);
  const [selectedSysUtilPrompt, setSelectedSysUtilPrompt] =
    useState<SysUtilPrompt | null>(() => {
      // Load selected prompt from local storage on initial render
      const stored = safeLocalStorage.getItem(
        SELECTED_SYSUTIL_PROMPT_STORAGE_KEY
      );
      if (stored) {
        try {
  ...
  // Watch for changes in SysUtil selection if needed
  useEffect(() => {
    const checkUserPromptSelection = () => {
      const userSelectedPromptId = safeLocalStorage.getItem(
        STORAGE_KEYS.SELECTED_SYSTEM_PROMPT_ID
      );
      // If user has selected their own prompt, clear SysUtil prompt selection
      if (userSelectedPromptId) {
        setSelectedSysUtilPrompt(null);
      }
    };
  ...
  const fetchSysUtilPrompts = async () => {
    setIsLoading(true);
    setError(null);
    try {
      const response = await invoke<SysUtilPromptsResponse>("fetch_prompts");
      setPrompts(response.prompts);
      if (response.last_updated) {
        setLastUpdated(response.last_updated);
      }
    } catch (err) {
      console.error("Failed to fetch SysUtil prompts:", err);
      setError(
        typeof err === "string" ? err : "Failed to fetch SysUtil prompts"
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleSelectSysUtilPrompt = async (prompt: SysUtilPrompt) => {
    ...
    try {
      ...
      setSelectedSysUtilPrompt(prompt);
      ...
      // Save the selected SysUtil prompt to local storage for persistence
      ...
      if (matchingModel) {
        ...
        await invoke("secure_storage_save", {
          items: [
            {
              key: SELECTED_SYSUTIL_MODEL_STORAGE_KEY,
      ...
      console.error("Failed to select SysUtil prompt:", error);
    }
  };

  const handleCardClick = (prompt: SysUtilPrompt) => {
    handleSelectSysUtilPrompt(prompt);
  };

  const isPromptSelected = (prompt: SysUtilPrompt) => {
    return (
      selectedSysUtilPrompt?.title === prompt.title &&
      selectedSysUtilPrompt?.modelId === prompt.modelId
    );
  };
    ...
    return (
      <div className="space-y-4 mt-6">
        ...
        <Empty
          ...
          description="Fetching SysUtil default prompts"
        />
      </div>
    );
  }
