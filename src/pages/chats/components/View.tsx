...
const View = () => {
  ...
  const handleSubmitWithTopic = (speechText?: string) => {
    if (activeTopic) {
      const raw = speechText || completion.input;
      const contextPrefix = `[Topic context: ${activeTopic} interview practice] `;
      completion.submit(contextPrefix + raw);
    } else {
      completion.submit(speechText);
    }
  };

  const handleKeyPressWithTopic = (e: React.KeyboardEvent) => {
    if (e.key === "Enter" && !e.shiftKey) {
      e.preventDefault();
      if (!completion.isLoading && completion.input.trim()) {
        handleSubmitWithTopic();
      }
    }
  };
            ...
            {completion.isRecording ? (
              ...
              <>
                ...
                <Textarea
                  ...
                  onKeyDown={handleKeyPressWithTopic}
