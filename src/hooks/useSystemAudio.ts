...
export function useSystemAudio() {
  ...
  const processWithAIRef = useRef<typeof processWithAI | null>(null);
  const useSystemPromptRef = useRef(useSystemPrompt);
  const systemPromptRef = useRef(systemPrompt);
  const contextContentRef = useRef(contextContent);
  const topicPromptRef = useRef<string | null>(null);

  // Keep refs in sync with latest state so listeners never have stale closures
  ...
  useEffect(() => { screenshotImageRef.current = screenshotImage; }, [screenshotImage]);
      ...
      try {
        const unlisten = await listen("speech-detected", async (event) => {
          try {
            ...
            if (!currentSttProvider.provider && !useSysUtilAPI) {
            ...
            try {
              ...
              if (transcription.trim()) {
                ...
                const fillerOnly = /^(hmm+|uh+|um+|ah+|oh+|okay|ok|yes|no|yeah|yep|nope|right|sure|alright|hi|hey|hello|thanks|thank you|bye|goodbye)\.?$/i.test(transcription.trim());

                // Ignore background noise: require at least 5 words and not just filler sounds
                if (words.length < 5 || fillerOnly) {
                  // Silently skip — background noise or filler, not a real question
                } else {
                  setLastTranscription(transcription);
                  setError("");

                  const baseSystemPrompt = useSystemPromptRef.current
                    ? systemPromptRef.current || DEFAULT_SYSTEM_PROMPT
                    : contextContentRef.current || DEFAULT_SYSTEM_PROMPT;
                  const effectiveSystemPrompt = topicPromptRef.current
                    ? `${topicPromptRef.current}\n\n${baseSystemPrompt}`
                    : baseSystemPrompt;

                  // Trim history to the most recent MAX_HISTORY_MESSAGES to prevent
                  // token usage from growing unboundedly after an hour of use.
                  const previousMessages = conversationRef.current.messages
        ...
        // If cleanup already ran before listen resolved, immediately unlisten
  ...
  const setTopicPrompt = useCallback((prompt: string | null) => {
    topicPromptRef.current = prompt;
  }, []);
  ...
  const processWithAI = useCallback(
    async (
      ...
      try {
        ...
        // Consume screenshot (if one was attached) and clear it
        const pendingScreenshot = screenshotImageRef.current;
        ...
        try {
          for await (const chunk of fetchAIResponse({
            ...
            imagesBase64: pendingScreenshot ? [pendingScreenshot] : [],
          })) {
            fullResponse += chunk;
        ...
        setIsAIProcessing(false);
        // Auto-restart capture after processing so next question is captured automatically
        if (capturingRef.current && vadConfig.enabled) {
          try {
            ...
            console.error("Failed to auto-restart capture:", restartErr);
          }
        }
      }
    },
    [selectedAIProvider, allAiProviders, capturing, vadConfig, selectedAudioDevices.output.id]
  );

  // Keep processWithAIRef in sync so the stable speech-detected listener always calls the latest version
  ...
  return {
    ...
    scrollAreaRef,
    // Screenshot for next AI message
    screenshotImage,
    setScreenshotImage,
    // Allow external callers (mic override) to update transcription display
    setLastTranscription,
    // Topic prompt override for focused interview practice
    setTopicPrompt,
  };
}
