...
export const useCompletion = () => {
  ...
  // Response history for Shift+A (prev) / Shift+S (current) navigation
  const [responseHistory, setResponseHistory] = useState<string[]>([]);
  const [historyIndex, setHistoryIndex] = useState<number>(-1); // -1 = show live response
  const inputRef = useRef<HTMLInputElement | null>(null);
  const isProcessingScreenshotRef = useRef(false);
  const screenshotConfigRef = useRef(screenshotConfiguration);
  const hasCheckedPermissionRef = useRef(false);
  const screenshotInitiatedByThisContext = useRef(false);
  // Tracks last speech-to-text transcription for screenshot+audio pairing
  ...
  const screenshotPendingRef = useRef<{ base64: string; autoPrompt: string } | null>(null);
  const [isAwaitingVoiceForScreenshot, setIsAwaitingVoiceForScreenshot] = useState(false);
  ...
  const submit = useCallback(
    async (speechText?: string) => {
      ...
      try {
        // Prepare message history for the AI — cap at MAX_HISTORY_MESSAGES to
        // prevent unbounded token growth after a long session.
        ...
        if (!selectedAIProvider.provider && !useSysUtilAPI) {
          setState((prev) => ({
            ...prev,
        ...
        // If an auto-speech submission interrupts the user while they are reading a completed 
        // live response, we seamlessly transition their view to the history version of that response 
        // to prevent their screen from wiping unexpectedly.
        setState((prev) => {
          if (speechText && prev.response && !prev.isLoading) {
            setResponseHistory((history) => {
              if (history.length > 0) {
                setHistoryIndex((prevIdx) => (prevIdx === -1 ? history.length - 1 : prevIdx));
              }
              return history;
            });
          }
        ...
        try {
          ...
          for await (const chunk of fetchAIResponse({
            provider: useSysUtilAPI ? undefined : provider,
        ...
        if (fullResponse) {
          // Push every response (typed or speech) to navigable history for Shift+A/S
          setResponseHistory((prev) => [...prev, fullResponse]);
          // Only jump to live view if user is NOT currently browsing history
          // This prevents yanking them away from a previous answer they're reading
          ...
          // Store last user message as transcription anchor for screenshots
          lastTranscriptionRef.current = input;
  ...
  useEffect(() => {
    ...
    const handleStorageChange = async (e: StorageEvent) => {
      if (e.key === "sysutil-conversation-selected" && e.newValue) {
        try {
  ...
  const handleScreenshotSubmit = useCallback(
    async (base64: string, prompt?: string) => {
      ...
      // Build enriched prompt: always pair screenshot with last spoken transcription
      const lastSpoken = lastTranscriptionRef.current;
      const enrichedPrompt = lastSpoken
        ? `[Interviewer's spoken question]: "${lastSpoken}"\n\n[Screenshot attached — treat as a coding question or find-the-error task. Use the format: UNDERSTAND → BRUTE FORCE → OPTIMAL → EXPLANATION]:${prompt ? `\n\nAdditional context: ${prompt}` : ""}`
        : prompt;
        ...
        if (enrichedPrompt) {
          ...
          try {
            ...
            for await (const chunk of fetchAIResponse({
              ...
              systemPrompt: systemPrompt || undefined,
              history: messageHistory,
              userMessage: enrichedPrompt || prompt || "Analyze this screenshot",
              imagesBase64: [base64],
              signal,
            })) {
              // Only update if this is still the current request
            ...
            // Save the conversation after successful completion
            if (fullResponse) {
              // Push to response history for Shift+A/S navigation
              setResponseHistory((prev) => [...prev, fullResponse]);
              setHistoryIndex((prev) => (prev === -1 ? -1 : prev));
              await saveCurrentConversation(enrichedPrompt || prompt || "Screenshot", fullResponse, [
                attachedFile,
              ]);
  ...
  const captureVoiceForScreenshot = useCallback(
    (base64: string, autoPrompt: string) => {
      screenshotPendingRef.current = { base64, autoPrompt };
      setIsAwaitingVoiceForScreenshot(true);
    },
    []
  );

  const onScreenshotVoiceCaptured = useCallback(
    async (voiceText?: string) => {
      setIsAwaitingVoiceForScreenshot(false);
      const pending = screenshotPendingRef.current;
      screenshotPendingRef.current = null;
      if (!pending) return;
      const combinedPrompt = voiceText?.trim()
        ? `${pending.autoPrompt}\n\n[My voice]: ${voiceText.trim()}`
        : pending.autoPrompt;
      await handleScreenshotSubmit(pending.base64, combinedPrompt);
    },
  ...
  const captureScreenshot = useCallback(async () => {
    ...
    try {
        ...
        if (!hasPermission) {
          ...
          if (!hasPermissionNow) {
            setState((prev) => ({
              ...prev,
              error:
                "Screen Recording permission required. Please enable it by going to System Settings > Privacy & Security > Screen & System Audio Recording. If you don't see SysUtil in the list, click the '+' button to add it. If it's already listed, make sure it's enabled. Then restart the app.",
            }));
      ...
      if (config.enabled) {
        ...
        if (config.mode === "auto") {
          // Auto mode: capture voice for 10s then send prompt + voice + screenshot
          screenshotInitiatedByThisContext.current = false;
          await captureVoiceForScreenshot(base64 as string, config.autoPrompt || "");
  ...
  }, [handleScreenshotSubmit, captureVoiceForScreenshot]);
  ...
  return {
    ...
    captureScreenshot,
    isScreenshotLoading,
    isAwaitingVoiceForScreenshot,
    onScreenshotVoiceCaptured,
    keepEngaged,
    setKeepEngaged,
    // Response history navigation (Shift+A = prev, Shift+S = current)
    responseHistory,
    historyIndex,
    setHistoryIndex,
  };
