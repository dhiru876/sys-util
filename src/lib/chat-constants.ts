 ...
 *
 * How frequently to check for new streaming chunks from SysUtil API.
 *
 ...
 *
 * Maximum number of messages (user + assistant combined) to include in the
 * conversation history sent to the AI provider on each request.
 *
 * After 1 hour of use, conversation history can grow to 60+ messages.
 * Sending the full history on every call exponentially increases token usage
 * and causes Claude Sonnet's hourly rate limit to be hit.
 *
 * 10 messages = 5 Q&A pairs, which provides sufficient context for interview
 * follow-up questions without flooding the API with stale history.
 */
