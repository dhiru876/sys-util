...
// SysUtil STT function
async function fetchSysUtilSTT(audio: File | Blob): Promise<string> {
  try {
    ...
    return `SysUtil STT Error: ${errorMessage}`;
  }
}
...
export async function fetchSTT(params: STTParams): Promise<string> {
  ...
  try {
    ...
    if (useSysUtilAPI) {
      return await fetchSysUtilSTT(audio);
    }
    ...
    if (isForm) {
      ...
      const audioExt = audio.type.includes("wav") ? "wav"
        : audio.type.includes("mp4") ? "mp4"
        : audio.type.includes("ogg") ? "ogg"
        : audio.type.includes("webm") ? "webm"
        : audio.type.includes("mpeg") || audio.type.includes("mp3") ? "mp3"
        : "wav";
      form.append("file", freshBlob, `audio.${audioExt}`);
