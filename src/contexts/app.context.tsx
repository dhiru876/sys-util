...
export const AppProvider = ({ children }: { children: ReactNode }) => {
  ...
  // SysUtil API State
  const [sysutilApiEnabled, setSysutilApiEnabledState] = useState<boolean>(false);

  const getActiveLicenseStatus = async () => {
    setHasActiveLicense(true);
    setSysutilApiEnabled(false);
  ...
  const loadData = () => {
    ...
    const nvidiaDefaults = {
      api_key: "YOUR_NVIDIA_API_KEY_HERE", // Get your key from: https://build.nvidia.com
      model: "meta/llama-3.3-70b-instruct",
    };

    if (savedSelectedAi) {
      const parsed = JSON.parse(savedSelectedAi);
      // Migration: if NVIDIA is selected but variables are empty (from when key was hardcoded),
      // inject the defaults automatically so nothing breaks
      if (parsed.provider === "nvidia" && !parsed.variables?.api_key) {
        parsed.variables = { ...nvidiaDefaults, ...parsed.variables };
      ...
      // Auto-select nvidia as default AI provider on first launch with pre-filled credentials
      const defaultAi = { provider: "nvidia", variables: nvidiaDefaults };
    ...
    if (savedSelectedStt) {
      ...
      // Auto-select deepgram-stt as default STT provider on first launch
      const defaultStt = { provider: "deepgram-stt", variables: {} };
    ...
    // Load SysUtil API enabled state
    const savedSysutilApiEnabled = safeLocalStorage.getItem(
      STORAGE_KEYS.SYSUTIL_API_ENABLED
    );
    if (savedSysutilApiEnabled !== null) {
      setSysutilApiEnabledState(savedSysutilApiEnabled === "true");
    }
  ...
  useEffect(() => {
    const checkImageSupport = async () => {
      if (sysutilApiEnabled) {
        // For SysUtil API, check the selected model's modality
        try {
          const storage = await invoke<{
            selected_sysutil_model?: string;
          }>("secure_storage_get");

          if (storage.selected_sysutil_model) {
            const model = JSON.parse(storage.selected_sysutil_model);
    ...
    checkImageSupport();
  }, [sysutilApiEnabled, selectedAIProvider.provider]);
  ...
  const setSysutilApiEnabled = async (enabled: boolean) => {
    setSysutilApiEnabledState(enabled);
    safeLocalStorage.setItem(STORAGE_KEYS.SYSUTIL_API_ENABLED, String(enabled));
    ...
    if (enabled) {
      try {
        ...
        console.debug("Failed to check SysUtil model image support:", error);
        setSupportsImages(false);
      }
    } else {
